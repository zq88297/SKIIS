...
**不要盲目加载所有上下文。先判断用户意图，再按需加载。**

#### 普通项目（无子模块，无多分支）

直接按规则 0 执行：检查当前目录的 `docs/ai-context/`，有就读，没有就初始化。

#### 大型多模块项目（根目录下有多个子功能目录）
...
用户提问
  │
  ├─ 步骤 1：只扫一级目录名（1 秒），不读文件内容
  │     ├─ 有架构文档缓存 → 读 architecture.md 的子模块清单
  │     └─ 无缓存 → ls 顶层目录，识别子模块
  │
  ├─ 步骤 2：根据用户问题，判断涉及哪个子模块
  │     例如：用户说"IPsec 隧道协商超时"
  │     → AI 判断：涉及 ike-module/ 和 key-exchange/
  │
  ├─ 步骤 3：检查涉及子模块的分支情况
  │     ├─ 单分支（只有一个目录或 Git 主分支） → 直接加载
  │     ├─ 多分支（branches/ 下有多个，或 Git 多分支） → 询问用户
  │     │
  │     │     "检测到 ike-module 有以下分支：
  │     │        • ike-module/trunk/
  │     │        • ike-module/branches/feature-ipv6/
  │     │        • ike-module/branches/bugfix-retry/
  │     │      需要排查哪个分支？"
  │     │
  │     └─ 用户指定分支 → 只加载该分支的上下文
  │
  └─ 步骤 4：加载目标分支的上下文 → 开始工作
```

**其他子模块的上下文不加载、不创建、不扫描。** 只有用户明确提到或 AI 判断相关时才碰。

#### 对话中途涉及新模块

如果排查过程中发现还涉及另一个子模块 B：

```
排查 ike-module 中发现涉及 key-exchange
  │
  ├─ 同样检查 key-exchange 的分支
  ├─ 有多个分支 → 询问用户
  └─ 单分支 → 自动加载，不询问
```

这项规则**不需要用户输入任何命令**。不管用户第一句话是什么，智能判断后按需加载。
