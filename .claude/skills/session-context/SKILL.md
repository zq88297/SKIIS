...
当在 A 的会话中直接排查了 B 的代码并得出**明确结论**时（修了 bug、确认了根因、排除了嫌疑），必须将结论回写到 B 的上下文，否则 B 那边完全不知道发生了什么：

1. **有结论** → 更新 B 的 `current-task.md` 追加排查结果
2. **发现了 bug** → 追加到 B 的 `pitfalls.md`
3. **做了技术决策** → 追加到 B 的 `decisions.md`
4. **任务完成** → 将 B 的 `tasks/from-A-xxx.md` 移到 `tasks/done/`，在文件末尾追加结论
5. **没结论** → 不写 B，只更新 A 的 current-task.md（记录排查过程，等下次继续）

写入后告知用户："已将排查结论同步到 B 的上下文文件"。
