...
**每次对话开始时，在回复用户问题之前，必须先自动执行：**

1. 检查 `docs/ai-context/current-task.md` 是否存在
2. **存在** → 静默读取，在回复中简要呈现：当前任务进度 + `🔜 下次继续` 段落 + 最近决策
3. **不存在** → 立即执行规则 5（自动初始化），然后再回复用户
4. 读完上下文后直接回答用户的问题，**不需要先说"上下文已加载"**——用户关心的不是加载过程，而是答案

这项规则**不需要用户输入任何命令**。不管用户第一句话是什么，都先读上下文。
